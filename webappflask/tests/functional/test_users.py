...
def test_valid_login_logout(test_client, init_database):
    ...
    response = test_client.post('/login',
                                data=dict(email='user1@gmail.com', password='testing'),
...
def test_invalid_login(test_client, init_database):
    ...
    """
    response = test_client.post('/login',
                                data=dict(email='user1@gmail.com', password='WrongPassword'),
                                follow_redirects=True)
    assert response.status_code == 200
...
def test_valid_registration(test_client, init_database):
    ...
    response = test_client.post('/register',
                                data=dict(email='user1@yahoo.com',
                                          password='testing',
                                          confirm='testing'),
                                follow_redirects=True)
    assert response.status_code == 200
...
def test_invalid_registration(test_client, init_database):
    ...
    """
    response = test_client.post('/register',
                                data=dict(email='user1@hotmail.com',
                                          password='testing',
                                          confirm='WrongPassword'),   # Does NOT match!
                                follow_redirects=True)
    assert response.status_code == 200
